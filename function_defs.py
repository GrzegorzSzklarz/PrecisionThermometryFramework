...
# --- CHEBYSHEV MODEL REGISTRATIONS ---

# 1. Standard Chebyshev Polynomial
@register_fitting_function("Chebyshev N-th degree polynomial", scaling_type='linear', is_polynomial=True)
def polynomial_chebyshev(x, *params):
    """Standard Chebyshev polynomial fit using linear [-1, 1] scaling."""
    return _evaluate_chebyshev(x, params)


# 2. Hybrid Chebyshev Polynomial + Sine
@register_fitting_function("Chebyshev N-th degree polynomial + Sine", scaling_type='linear', is_polynomial=True)
def hybrid_sine_chebyshev(x, *params):
    """Hybrid Chebyshev polynomial + Sine wave model using linear [-1, 1] scaling."""
    return _evaluate_hybrid_sine_chebyshev(x, params)


# 3. Chebyshev Rational Model (Special Workflow)
@register_fitting_function("Chebyshev Rational Function", is_special_workflow=True)
def create_chebyshev_rational_function(n_degree, m_degree, b0_is_zero):
    """
    Factory function for Chebyshev Rational models: P_n(x) / Q_m(x)
    where both numerator and denominator use Chebyshev basis functions T_i(x).
    """
    def chebyshev_rational_func(x, *params):
        ...
        # Numerator in Chebyshev basis
        numerator = _evaluate_chebyshev(x, p_coeffs)
        
        # Denominator in Chebyshev basis: 1 + sum(h_i * T_p(x))
        offset = 1 if b0_is_zero else 0
        powers = np.arange(offset, len(h_coeffs) + offset)
        
        denominator = 1.0 + sum(h * chebval(x, [0]*p + [1]) for p, h in zip(powers, h_coeffs))
        denominator = np.where(np.abs(denominator) < 1e-12, 1e-12, denominator)
        
        return numerator / denominator
    return chebyshev_rational_func
    
# 4. Log-scaled Chebyshev Polynomial
@register_fitting_function("Log-scaled Chebyshev N-th degree", scaling_type='log', is_polynomial=True)
def polynomial_chebyshev_log_scaled(x_norm, *params):
    """
    Chebyshev polynomial fit on logarithmically scaled data.
    Expects x_norm mapped from ln(R) to [-1, 1].
    """
    return _evaluate_chebyshev(x_norm, params)


# 5. Log-scaled Hybrid Chebyshev Polynomial + Sine
@register_fitting_function("Log-scaled Chebyshev N-th degree + Sine", scaling_type='log', is_polynomial=True)
def hybrid_sine_chebyshev_log_scaled(x_norm, *params):
    """
    Hybrid Chebyshev polynomial + Sine wave model on logarithmically scaled data.
    Expects x_norm mapped from ln(R) to [-1, 1].
    """
